...
void adamic_adar(char *in_dir, char *out_dir, bool use_val){
	...
	for(int i = 0; i < num_nodes; i++){
		...
		if(deg[i] > 0)score[i] = 1.0 / log(deg[i]);
	}
	freopen((string(out_dir) + string("adamic_adar_train_pos.txt")).c_str(), "w", stdout);
	for(int i = 0; i < train_pos_m; i++){
	...
	fclose(stdout);
	freopen((string(out_dir) + string("adamic_adar_train_neg.txt")).c_str(), "w", stdout);
	scanf("%d", &train_neg_m);
	...
	fclose(stdout);
	freopen((string(out_dir) + string("adamic_adar_valid_pos.txt")).c_str(), "w", stdout);
	scanf("%d", &valid_pos_m);
	...
	fclose(stdout);
	freopen((string(out_dir) + string("adamic_adar_valid_neg.txt")).c_str(), "w", stdout);
	scanf("%d", &valid_neg_m);
	...
	freopen((string(out_dir) + string("adamic_adar_test_pos.txt")).c_str(), "w", stdout);
	scanf("%d", &test_pos_m);
	...
	fclose(stdout);
	freopen((string(out_dir) + string("adamic_adar_test_neg.txt")).c_str(), "w", stdout);
	scanf("%d", &test_neg_m);
