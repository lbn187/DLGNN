...
def train(model, predictor, emb, data, split_edge, train_pos_edge_info, node_info, optimizer, batch_size, ratio, device):
...
def main():
    ...
    for extra_data in args.extra_data_list:
        ...
        f.close()
        f = open(args.extra_data_dir + extra_data + "_node.txt", "r")
        lines = f.readlines()
        ret = [float(x) for x in lines]
        tmp_node_info = torch.FloatTensor(np.array(ret).reshape(-1, 1))
        f.close()
        ...
        node_info_list.append(tmp_node_info * weight)
    if len(args.extra_data_list) > 0:
        ...
        node_info = torch.cat(node_info_list, dim = 1)
    else:
        ...
        node_info = torch.FloatTensor(data.num_nodes, 1)
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, predictor, emb.weight, data, split_edge, train_pos_edge_info, node_info, optimizer, args.batch_size, args.negative_sample_ratio, device)
